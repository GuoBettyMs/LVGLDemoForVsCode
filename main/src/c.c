...
/**********************/
#define   WAVE_SIZE										600
static lv_coord_t g_asDispPoints[WAVE_SIZE/2];
static lv_coord_t g_asTempDispPoints[WAVE_SIZE/2];

static void ClearChartDisplayBuffer(void);

/**********************
 *   GLOBAL VARIABLES
 **********************/
PageDsc_t	g_tPageDsc;
PGDeskData_t g_tDeskData;
static ChannelShowData_t g_tChannelShowData[POWER_CHANNEL_ALL];
// static UIBatteryState_t g_atUIBatteryState[POWER_CHANNEL_ALL] = {BATT_STATE_IDLE};
static AutoCharging_t	g_tAutoCharging;
...
//     lv_obj_set_style_text_font(title, is_top ? &font_isdtyahei_20 : &font_bahnschrift_18, 0);
...
//     lv_obj_set_style_text_font(title_time, is_top ? &font_bahnschrift_20 : &font_bahnschrift_18, 0);
//     if(is_top){
...
// 事件处理函数
void play_btn_event_handler(lv_event_t* e) {
    lv_event_code_t code = lv_event_get_code(e);
    lv_obj_t* btn = lv_event_get_target(e);

    if (code == LV_EVENT_CLICKED) {
        LV_LOG_USER("Play button clicked. Destroying chart...");

        // 调用在 custom_chart.c 中定义的清理函数
        // free_line_chart_resources();
